...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lighting_shader.use();
		lighting_shader.set_vec3("light.position", lightPos);
...
void mouse_callback(GLFWwindow* window, const double xposIn, const double yposIn)
{
	...
	const float x_offset = xpos - lastX;
	const float y_offset = lastY - ypos; // reversed since y-coordinates go from bottom to top
