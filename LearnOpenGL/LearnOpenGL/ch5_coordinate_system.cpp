...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		auto projection_trans = glm::mat4(1.0f);
		const float view_distance = -3.0f;
		view_trans = translate(view_trans, glm::vec3(0.0f, 0.0f, -3.0f));
		projection_trans = glm::perspective(glm::radians(55.0f), static_cast<float>(SCR_WIDTH) / static_cast<float>(SCR_HEIGHT), 0.1f, 100.0f);
		// retrieve the matrix uniform locations
		const unsigned int view_loc = glGetUniformLocation(our_shader.id, "view");
		// pass them to the shaders
		glUniformMatrix4fv(view_loc, 1, GL_FALSE, &view_trans[0][0]); // 2.pass the ref
		// NOTE: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
		our_shader.set_mat4("projection", projection_trans); // 3.use my "shader" class
